                                                        ...
                                                        <div class="nav accordion" id="accordionSidenav">
                                                            ...
                                                            <div class="collapse" id="<?php echo sanitizeID($row['class_name']) ?>" data-bs-parent="#accordionSidenav">
                                                                <nav class="sidenav-menu-nested nav">
                                                                    ...
                                                                    $users2 = $conn->query("SELECT * FROM `tblsection` a INNER JOIN tblclasses b  ON a.section_class = b.class_id WHERE b.class_id = '$classID' AND a.section_user = '$user_id' ");
                    ...
                    viewattendace.addEventListener("click", () => {
                        ...
                        axios({
                            ...
                            if (response.data != "0") {
                                ...
                                // Clear existing rows
                                const headerRow = $("<tr>");
                                headerRow.append("<th style='text-align:center'>Student ID</th>");
                                ...
                                studentInformation.forEach((user) => {
                                    const studentKey = user.idnumber;

                                    if (!studentDataMap.has(studentKey)) {
                                        // Initialize student data if it doesn't exist in the Map
                                        studentDataMap.set(studentKey, {
                                            idnumber: user.idnumber,
                                            name: `${user.lastname}, ${user.firstname} ${user.middlename}.`,
                                            attendanceStatus: new Array(uniqueDates.length).fill(""),
                                            totalPresent: 0,
                                            totalAbsent: 0,
                                            totalLate: 0,
                                        });
                                    ...
                                    // Update attendance status for the corresponding date
                                    const dateIndex = uniqueDates.indexOf(user.attendance_date);
                                    if (dateIndex !== -1) {
                                        studentData.attendanceStatus[dateIndex] =
                                            user.attendance_status === 1 ? "present" :
                                            user.attendance_status === 2 ? "late" : "absent";
                                    }

                                    // Update total counts
                                    if (user.attendance_status === 1) {
                                        studentData.totalPresent++;
                                    } else if (user.attendance_status === 2) {
                                ...
                                studentDataMap.forEach((studentData) => {
                                    const row = `
                    <tr style="text-align:center">
                        <td>${studentData.idnumber}</td>
                        <td>${studentData.name}</td>
                        ${studentData.attendanceStatus.map(status => `<td>${status}</td>`).join('')}
                        <td>${studentData.totalPresent}</td>
                        <td>${studentData.totalAbsent}</td>
                        <td>${studentData.totalLate}</td>
                    </tr>
                `;
                                    tableBody.append(row);
                                });
            ...
            `;
                ...
                <div class="modal fade" id="qr-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" style="max-width: 680px;"> <!-- Set a custom max-width value -->
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <input type="text" id="rfidText" class="form-control" placeholder="Please Scan your QR code at the Scanner">
                            </div>
                            <div class="modal-footer">
                ...
                <script>
                    document.addEventListener("DOMContentLoaded", function() {
                        ...
                        document.getElementById("rfidText").addEventListener("keyup", (e) => {
                            if (e.key === "Enter") {
                                // Fetch from the database using e.target.value which is the emp ID
                                var classIDni = document.getElementById("classIDni").value;
                                var sectionIDni = document.getElementById("sectionIDni").value;
                                console.log("QR Code Value:", e.target.value);
                                console.log("classIDni", classIDni);
                                console.log("sectionIDni", sectionIDni);
                                if (e.target.value && classIDni && sectionIDni) {
